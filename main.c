/* Simple program that uses Huffman trees to encode files
...
int main()
{
	...
	puts("Insert name of input file (input.txt).");
	fflush(stdin);
	scanf("%s", infile);
	puts("Insert name of output file (output.txt).");
	fflush(stdin);
	scanf("%s", outfile);
	puts("Insert 1 to Encode file, Insert 2 to Decode file.");
	scanf("%d", &compress);
